...
**Why does `range(1,5)` include 1 but not 5?** Python uses "half-open" intervals: include the start, exclude the end. This means `range(n)` gives exactly `n` items, and consecutive ranges like `range(0,5)` + `range(5,10)` don't overlap or leave gaps. The same rule applies to list slicing: `x[1:3]` gives items at index 1 and 2, not 3.

--

**What does "vectorized" mean?** In R, `c(1,2,3) + 10` gives `c(11,12,13)` — the operation applies to every element automatically. That's vectorization. In Python, `[1,2,3] + 10` is an error — plain lists don't do element-wise math. You need NumPy or pandas for that (covered in Module 2).
